...
def main_loop(db, layer_sizes, learning_rate, epochs=100):
    ...
    particles = [np.random.randn(sf.calc_weight_vec_length(layer_sizes)) for i in range(10)]

    p_best_pos = deepcopy(particles) # Stores list of best positions
    p_best_scores = [] # Stores list of best scores
    ...
    for e in range(epochs):
        for i,p in enumerate(particles):
            ...
            # caluclate velocity
            v[i] = velocity(v[i], p_best_pos[i], g_best_pos[i])
            p[i] = p[i] + v[i]
